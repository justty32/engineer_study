...
- 以手機實際開啟[工程學習中央入口](https://justty32.github.io/engineer_study/)，確認 16 個課程入口可點擊；進入任一章節後再按「回到工程學習主站」，確認按鈕位置、觸控範圍與返回行為符合預期。目前內建瀏覽器控制介面因環境設定不完整而無法建立連線，靜態與 HTTP 導覽檢查已由 agent 完成。
