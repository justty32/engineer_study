...
- 以桌面 `1440×900` 與手機 `390×844` 實際開啟[機器人路徑與運動規劃](../專題/機器人路徑規劃/互動課程/index.html)、[機器人視覺](../專題/機器人視覺/互動課程/index.html)與[機器人學習與訓練](../專題/機器人學習/互動課程/index.html)，確認各 9 頁導覽、字典搜尋、所有自由實驗、鍵盤焦點及瀏覽器主控台沒有錯誤、重疊或水平溢位；本次內建瀏覽器控制介面因環境層缺少必要設定而無法建立連線，故只有靜態與公式驗收證據。
- 以桌面 `1440×900` 與手機 `390×844` 實際開啟[七軸機械手臂運動學互動課程](../專題/機器人運動學/互動課程/index.html)，確認 13 頁導覽、字典搜尋、座標／2R／旋轉／變換／七軸 FK／雅可比／零空間／奇異點／數值 IK／C++ 契約互動、鍵盤焦點與瀏覽器主控台沒有錯誤、重疊或水平溢位；目前瀏覽器控制介面無法建立連線，故只有靜態與公式驗收證據。
