...
**實際的收件匣在 [`wf/inbox/`](../../inbox/)**。本工作流只說明怎麼使用；收件匣本身只放信件並保持乾淨。
...
> 這是可選工作流。沒有多方 agent 交接時，保持空收件匣即可。

## 地址 = inbox 資料夾位置

**用「在哪個資料夾工作」來認 agent。** 一個 agent 的地址，就是它工作資料夾內約定的 `inbox/`；本專案的地址是 [`wf/inbox/`](../../inbox/)。
...
> 放信處是 [`wf/inbox/`](../../inbox/)（含 `done/` 歸檔），不在本資料夾。本入口只保留使用方式；膨脹後依 [DEV-GUIDE「結構整理原則」](../../DEV-GUIDE.md) 拆分。
